...
# model = load('.', 'custom', path='best.pt', force_reload=True)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                LOGGER.info("Send signal to GPIO")
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if conf.item() > 0.50:
                        ...
                        file_path = "predictions_logs/{}_predictions.csv".format(int(time.time()))
                        data = pd.DataFrame.from_records(predicted_labels)
            ...
            predicted_labels = sorted(predicted_labels, key=lambda x: x["confidence"], reverse=True)
            if view_img:
